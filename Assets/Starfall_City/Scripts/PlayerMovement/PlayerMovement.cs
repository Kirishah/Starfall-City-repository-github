...
namespace Movement
{
    public class PlayerMovement : MonoBehaviour, QTEGameManager.IRPGComponent
    {
        ...
        void CheckIfReachedDestination()
        {
            ...
            if (_currentTargetInteractable != null && !_currentTargetInteractable.gameObject.activeInHierarchy)
            {
                _currentTargetInteractable = null;
                player.ResetPath();
                ClearDestinationIndicator();
        ...
        public Vector3 GetDesiredDirection()
        {
            ...
            return player.desiredVelocity.sqrMagnitude > 0.0001f
                ? player.desiredVelocity.normalized
                : Vector3.zero;
        }
    }
}
