...
public class DanceInput : MonoBehaviour
{
    ...
    {
        if (!QTEGameManager.IsQTEActive || config == null) return; // Exit early if QTE not active
    ...
    {
        if (!QTEGameManager.IsQTEActive || !IsHolding || currentHoldArrow == null || currentHoldArrow.direction != direction) return;
    ...
    private void Update()
    {
        if (!QTEGameManager.IsQTEActive || !IsHolding || currentHoldArrow == null || config == null) return;
