...
namespace DialogueSystem
{
    public class DialogueManager_UIToolkit : MonoBehaviour, QTEGameManager.IRPGComponent
    {
        ...
        [Header("UI Handler")]
        [SerializeField] private DialogueUI_Toolkit _uiHandler;
        ...
        public void StartDialogue(string startID, string npcID)
        {
            ...
            {
                Debug.LogError($"Dialogue with ID {startID} not found! (Or dialogue data base is missing)");
                return;
            }
            _uiHandler.ClearHistory();
            _uiHandler.ShowPanel();
        ...
        public void SelectChoice(string choiceText, string targetID, bool triggersQTE, int deltaPoints)
        {
            ...
            {
                _uiHandler.AddToHistory("You", choiceText, isPlayer: true);
            }

            // Accumulate points from this choice
            _currentDeltaPoints += deltaPoints;

            var selectedChoice = _currentDialogue?.choices?.Find(c =>
                c.text == choiceText &&
                (c.targetID ?? "") == (targetID ?? ""));
            ...
            if (triggersQTE)
            {
                var qteID = _currentDialogue?.qteID ?? "default_dance_battle";
        ...
        private void HandleActionChoice(string targetID)
        {
            ...
            {
                var questSO = Resources.Load<QuestSO>("Quests/" + parts[2]);
                if (questSO != null)
                {
        ...
        public void ShowDialogue(Dialogue dialogue)
        {
            ...
            _uiHandler.UpdateSpeakerAndIcon(dialogue.speaker, dialogue.iconPath);

            // Append to history
            if (!string.IsNullOrEmpty(dialogue.description))
            {
                _uiHandler.AddToHistory("", dialogue.description, isNarrative: true);
            }
            if (!string.IsNullOrEmpty(dialogue.speaker) && !string.IsNullOrEmpty(dialogue.text))
            {
                _uiHandler.AddToHistory(dialogue.speaker, dialogue.text);
            }

            OnDialogueLineDisplayed?.Invoke(dialogue.id, _currentNPCID);

            // Display choices or continue
            _uiHandler.DisplayChoices(
                 dialogue.choices,
                 dialogue,
            ...
            {
                _uiHandler.ShowContinueButton(() => HandleContinueAction(dialogue));
            }

            // Audio
            if (!string.IsNullOrEmpty(dialogue.audio))
            {
                var clip = Resources.Load<AudioClip>(dialogue.audio);
                if (clip != null)
                {
                    _audioSource.Stop();
        ...
        {
            if (dialogue == null || _uiHandler == null)
            {
        ...
        {
            var allQuests = Resources.LoadAll<QuestSO>("Quests");
