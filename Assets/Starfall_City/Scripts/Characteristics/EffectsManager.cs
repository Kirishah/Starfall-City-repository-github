...
public class EffectsManager : MonoBehaviour
{
    ...
    private Dictionary<string, DialogueEffectsConfig> effectsConfigs = new Dictionary<string, DialogueEffectsConfig>();
    ...
    {
        DialogueEffectsConfig[] allConfigs = Resources.LoadAll<DialogueEffectsConfig>("DialogueEffects/");
        foreach (var config in allConfigs)
        {
            if (!effectsConfigs.ContainsKey(config.configID))
            {
                effectsConfigs[config.configID] = config;
            }
        }
        Debug.Log($"Loaded {effectsConfigs.Count} effects configs from Resources.");
    }
    ...
    public void ApplyPoints(string effectType, int points)
    {
        ...
        if (effectsConfigs.TryGetValue(effectType, out var config))
        {
    ...
    {
        effectsConfigs[effectType] = config;
    ...
    {
        EffectsManager.Instance.ApplyPoints("kiryaStar", 3);
    }
}
